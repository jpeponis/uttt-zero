...
def line_after(cells, macro, nb, player, first_move, max_len=8):
    ...
    pv, end = pv_and_end(g.cells, g.macro, g.next_board, g.player, ch, max_len=max_len - 1)
    h = as_game(cells, macro, nb, player)
    for m in [int(first_move)] + pv:
        h.play(m)
        if h.done:
            break
    while not h.done:
        _, ch2 = solve_children((h.cells, h.macro, h.next_board, h.player))
...
def main() -> None:
    ...
    for i in idx:
        ...
        best_line, best_end, best_w = line_after(cells, macro, nb, player, int(z["pv"][i][0]))
        raw_line, raw_end, raw_w = line_after(cells, macro, nb, player, raw)
