...
## Handover (2026-09-13, 08:38 — written for the next instance; this one stopped at ≈ 64 % of its context at the owner's word)

**State.** **K1 is running** — `runs/deep8_c1_300_e8_draw`, the play agent's recipe under the plain-draw tiebreak as
its one change, launched 2026-09-13 00:20:02 at the owner's word (`wscript runs/launch_queue14_hidden.vbs`, from
`33ebd8a`; `config.json` records `rule draw`, `gumbel_scale 1.0` and the resolved search configuration). At 08:37:
iteration 123 of 300 on the 64-sim leg at ≈ 285 s per iteration → **≈ 22:40 today**; `end_count` 0.0 throughout;
draw share ≈ 47 % at iteration 123 (the parent's final ≈ 17 %; the mechanical relabel baseline 32.3 %); the E7 worker
(pid 4451) under `draw` on the 3060 (`eval_full_draw.jsonl`: net_0070 75.6 % vs v2b under draw, 42.6 vs the count
parent, 60.1 vs deep8_c1_300; raw WDL 89.8 on the draw set). When training reaches DONE, `runs/eval_run_k1.sh` runs
by itself (item 4's two cross-play matches against the parent, v2b under both rules, the 2 × 2 endgame reads →
`runs/deep8_c1_300_e8_draw/analysis.out`) and `runs/queue14.out` ends "queue14 done" with the worker's exit status; a
...
rebuttal, M1 and M1's rebuttal — all run at medium and adjudicated with every evidence cell re-derived (§7e: R1–R8,
M2 rows 1–18, M2-R R1–R13, M1 rows 1–67, M1-R R1–R14); M2's engineering and its rebuttal's built by worktree agents,
merged and tested (the K1 plumbing, the J3 / J4 tools, the five I1 tools threaded, the boundary suite, the launcher's
failure handling); the `_e8` count-rule pass run and read (37 claims: 27 held / 9 moved / 1 unresolved / 0 reversed;
7's reversal confirmed and deepened at 0.943 of the visits on the corner reply); K1's instruments built, tested and
frozen before its final checkpoint exists (`runs/plan7/K1_readings_3060.sh`, `tools/common_positions.py`,
`tools/paired_contrast.py`), and §5 item 2 amended to the difference-in-differences form on their control finding (a
count net's own Δ across the evaluation rules is −0.020 mechanically; two count nets' DiD 0.0000); the draw-rule
endgame sets built; K1 launched; J3 and J4 run and read (KNOWLEDGE 52, 53); the map, the literature draft, KNOWLEDGE
and `knowledge/07` brought to M1-R's state (31's causal headline gone; 33's columns unpaired; the HUJI report
CLOSED-DRAW with its 2 and 5 verbatim; the Padua thesis CLOSED, tiebreak unverified; the Othello comparison deleted).

**What the next instance does, in order.**

1. **Watch K1** (above): a glance at `runs/deep8_c1_300_e8_draw/log.jsonl` for `t_iter` and `draw` now and then;
   nothing else until ≈ 22:40.
2. **When K1 lands:** read `analysis.out` (the in-run table; s_C and s_D against the parent; the 2 × 2 endgame reads —
   diagnostic, not item 3). Then on the 3060, in order: `bash runs/plan7/K1_readings_3060.sh` (item 2, ≈ 5 h; its
   header states the corpus choice M3 judges — count-play positions read under draw, the draw net's own games only for
   the descriptive claims, `surprise` / `probe_value` on the draw run's own buffer); `tools/common_positions.py
   --corpus_a runs/deep8_c1_300_e8_draw --corpus_b runs/deep8_c1_300_e8 --net_a runs/deep8_c1_300_e8_draw/net_0300.pt
   --net_b runs/deep8_c1_300_e8/net_0300.pt --n 15000 --sims 256 --device cuda:1 --out
   runs/plan7/K1_common_positions.json` (item 3, and item 2's DiD — the count parent's own Δ is the control line);
   `tools/paired_contrast.py --count runs/deep8_c1_300_e8_draw/paired_vs_deep8c1_300e8_64.json --draw
   runs/deep8_c1_300_e8_draw/paired_vs_deep8c1_300e8_64_draw.json` (item 4). Then the reading: every §1–§8 claim
   marked *rule-invariant* / *rule-dependent* / *unresolved* by §5 item 2's rules as amended (the DiD intervals for 16,
   8, 13; the draw share against 32.3 % with a 2-point margin; claim 1's tie rule; 7's 0.02 visit-share tolerance at
   16 384 sims; 33–34), one KNOWLEDGE clause each, K1's row in README's ladder (Elo "n/a — a different game"; the
   endgame column its draw-set read), README's Current state, a log entry, commit. Delegate the reading to an opus
   `directed` agent with the outputs and `runs/plan7/K1_parent_reading.md` as the count baseline; read its table
   before merging. Every reading is "observed in this pair of runs".
3. **J5**, the manuscript skeleton (`docs/paper/paper.md` from the map's §3, which does not yet place 52 and 53),
   after K1's reading; then **M3**, the draft's review (brief on the M0 template, naming files; one review per window;
   its inherited open items: the Padua terminal rule, the HUJI year, `knowledge/06`'s inferred solving cost, the
   corpus choices in K1's readings, the DiD amendment of §5 item 2 for the reviewer's eye).
4. **§10's file updates** after K1: RETROSPECTIVE §7 and a §8 "what the write-up changed" (31's causal headline, 33's
   unpaired columns, 15's magnitudes, 7's confirmed reversal, the reuse framing); the explainer's Part 8. E11's copy
   when a destination exists. Phase L is the owner's call. §8 stays unproposed.
...
**Traps added this session (2026-09-13).** PLAN7 is CRLF since the M1 adjudicator's merge, and a scripted insert must
use the file's own line ending (thirteen doubled CRs were normalised at `0b599ce`). An agent's GPU smoke on `cuda:0`
slows K1 by 40 % — the trainer's card is off limits while it runs; the 3060 is shared freely. J4's 3.5 h estimate was
32 min (only the incomplete positions spend the budget). `runs/*/*.md` ledgers needed a `.gitignore` whitelist
(`!runs/*/*.md`, in). The pass's `principles` output uses the pre-merge key names. The Codex accounting at medium: a
scoped review ≈ 1 M tokens / 4–8 min, a resumed rebuttal ≈ 2–5 M (mostly cache) / 2–8 min, and ≈ 12 M input tokens
filled one five-hour window; a stage that hits the limit fails in 25 s with the reset time in the error event.
## Log
...
- **2026-09-12, 23:40 — M1 adjudicated in a worktree (§7e M1, 67 rows) and the accepted amendments applied.**
  The review of the account returned at 22:42 (`4235f00`): 498 s at medium, 2 905 159 input tokens of which
  2 683 904 cached, 12 168 output, 23 commands, **12 web searches**, 2 474 words — twice M2's cost and an
  eighth of M0's, for a review that fetched five sources and recomputed ten map entries. Adjudication:
  **55 accepted, 8 accepted with a change of remedy, 1 rejected, 3 noted.** The rejection is 31's diagnosis:
  the drift column records whether a reading recurs on a stronger net, not whether a mechanism is identified,
  and M0 row 2 already marks that half *inference* — but the cell's evidence was wrong and is corrected (its
  "≈ +6 on `_e4`" was the paired score against v2b, not draw recognition, which steps 70.6 → 74.7 across the
  first drop). What the review bought: R5's Wang correction and the Lovering comparator — its supplement
  lists **four** models, not one agent, with Appendix D replicating the key figures across architectures —
  both reached the literature draft at last; the map's staleness against committed KNOWLEDGE closed (J1a's
  sealed `_e8` read, 92.7 [91.8, 93.6] against the dev half's 91.6, and §1's already-qualified reversal
  note); the drift tally regenerated from explicit per-row assignments (it summed to **62 over 59 rows**; it
  is now sign or order changed 4 · magnitude changed 25 · unresolved 1 · compatible 18 · not measured 11,
  with "compound" an annotation on nine rows and row 42's G-CNN half re-bucketed, because four ResNet second
  drops of +2.6 / +4.1 / −0.4 / −0.3 resolve nothing to reverse); every *compatible* cell marked **[CI]** or
  **[no CI]**, since only six of the twenty-four rest on an interval; and five outline sentences replaced by
  the forms that survive a hostile reader. Three things it found that nobody had: claim 33's two columns are
  measured on *different* position sets (the agent's quarter on unsolved positions, the solver's 70.2 % on
  4 791 solved ones — and `_e4`'s value is deep8_300's 0.7021498643 to the last digit, the same set); claim
  32's two puzzle samples come from *different corpora* (deep10's from deep8_300's late games, `_e4`'s from
  deep10's), not "the same corpus"; and KNOWLEDGE 15's raw-head hierarchy has *not* been "unchanged to the
  second decimal since v2a" (1.026 / 0.910 / 0.794 then, 1.052 / 0.941 / 0.827 now — the ordering held, the
  magnitudes did not). Amended: sixteen KNOWLEDGE claims (2, 3, 14, 15, 17, 18, 19, 31, 31a, 32, 33, 35, 41a,
  48, 50, 51, each clause ending "(M1, 2026-09-12)"), `knowledge/07`'s Lovering entry and closing item 12,
  §2b's probing row, and both paper drafts. Six items go to M1's rebuttal round with their evidence (§7e's
...
- **2026-09-13, 08:37 — J3 and J4 read into KNOWLEDGE** (an opus worktree agent, merged `76366bd`). The two
  pre-registered readings land as a new KNOWLEDGE §9a: **52** (J3) and **53** (J4), in the forms §4 and §7e M2 rows
  3, 4, 7, 15, 16 / M2-R R5–R7 fixed before either ran. 52, *search-relative* on `_e8` and `_e4`: the root **+0.5242**
  / **+0.4950** at 16 384 sims with both principal lines — the same search and the same number claim 3 already
  carries for [40], written out, not a new point; the near-greedy split **99.7 % [99.3, 99.9]** over **9** distinct
  games and **99.9 %** over **7**, the interval the policy's with the concentration beside it and no opening
  coverage claimed; the as-trained split **70.9 / 12.6 / 16.6** and **69.8 / 13.5 / 16.8** (not play as trained);
  (b) − (c) the exploration package at a matched budget, four knobs unattributed (X +28.9 / +30.2); 24's corpus
  split 63.2 / 20.2 / 16.6 *compared*, not tested — X +7.7, O −7.6, **the draw share unmoved**; the raw-policy
  (0.982 / 0.990) against generated-games (0.835 / 0.841) first-move statistics, each labelled. 53, coverage
  *exact* and the optimal-move rate *search-relative against exact labels*, `_e8` only: complete legal-action value
  coverage within 10⁸ nodes **28.2 % [24.4, 32.3]** of games alive at ply 40 → **100 % [99.2, 100.0]** from 52; the
  agent optimal in **12 565 of 12 573** covered positions (eight non-optimal moves, one at each of plies 44–49, 51,
  52 — four draw → loss, three win → draw, one at ply 48 win → loss), median / p90 nodes at 40 / 45 / 50 / 60, the
  finite-population note at 66–70, §4 J4's sentence verbatim, the three caveats. No monotonicity, no frontier, no
  "solved from ply N". The map gains rows 52–53 (drift *not measured*), 61 rows / 58 claims; ledger
...
commands, 12 web searches; 2 474 words).** Verdicts as before. Every evidence cell below was
re-derived in this worktree before its verdict was written — the named `runs/` output opened at the
named lines, the number recomputed, the web source fetched. The `_e8` count-rule pass
(`runs/plan7/K1_parent_*.out`) was in flight throughout and none of its readings enters this table.

| # | review item | verdict | evidence / reason | lands in |
|---|---|---|---|---|
| F1 | Finding 1: R5's Wang correction never reached the draft (`02_literature.md:101–107`) — it still infers an optimum between the sweeps and omits their fixed-setting result | **accept** | Re-fetched here (arXiv HTML 2003.05988): §6.2 "generally, larger *m* and larger *ep* lead to higher Elo ratings", with "the Elo rating of *ep* = 10 is higher than that of *ep* = 15 for *m* = 75"; Fig. 6, at a fixed time budget, "more training within one iteration does not show improvement for Elo ratings"; Table 1's `rs` default 20, `ep` ∈ {5, 10, 15}, §5.2's I ∈ {25, 50, 75}; the game is 6×6 Othello. The draft's "consistent with an optimum between them" is the sentence R5 refused | `02_literature.md`, the sample-reuse paragraph: both of their findings, no inferred optimum |
| F2 | Finding 2: the probing comparator is understated — Lovering et al. is not "one trajectory" | **accept** | Fetched here (ar5iv HTML of arXiv:2211.14673): "For all results in the main body of the paper we use the model grubby. We report additional results with other models below in Appendix D"; the four models are grubby (8 layers, 512), recent (8, 256), baggy (4, 512), vital (2, 1024), agents trained by Jones 2021b, 9×9 Hex, 21 checkpoints; Appendix D "Additional Results — Fig. 14 reports key figures from the main paper replicated across architectures". The main body is one agent; the appendix replicates across three further *architectures*, which is not what this project's four independently trained nets are — the claim narrows, it does not disappear | `02_literature.md` (the Lovering sentence and the table row), `knowledge/07` (the Lovering entry and closing item 12), §2b's probing row |
| F3 | Finding 3: the map is stale against committed KNOWLEDGE — row 30's "open sealed gap" and §2 item 2's uncorrected note | **accept** | KNOWLEDGE 30 already carries J1a, re-read here at `runs/plan7/J1a_endgame_v3_test_e8.out:7` (sealed **92.7 [91.8, 93.6]**, draws 85.3 %, regret 0.027 [0.021, 0.034]) and `J1a_endgame_v3_dev_e8.out:7` (**91.6 [90.6, 92.6]**), with the 256-sim search at `:11` **99.9 [99.8, 100.0]** against **100.0 [100.0, 100.0]**. KNOWLEDGE `:65–70` already reads "No other *primary* sign or ordering changed", with the M0 note attached | map row 30, §2 items 1–2, §4a's row-30 line |
| F4 | Finding 4: the outline's prose drops the qualifications its rows carry (five sentences) | **accept** | Each surviving sentence re-derived in H1–H5 below | map §3 |
| F5 | Finding 5: row 14's reconciliation is wrong and its drift cell omits B3 | **accept** | deep10's own two fits span **+0.056 … +0.079** (A4 +0.0556 / +0.0652 / +0.0697, `plan5_A4_freemove_deep10_on_deep8late.out:36–41`; B3 +0.0662 / +0.0764 / +0.0791, `plan5_B3_value_deep10.out:36–41`), so "+0.03 … +0.08" is not "deep10's own two fits": +0.030 is deep8_300's A4 corner, and the range is the span over deep10 **and** deep8_300, the two nets the claim was first written from. R6's per-model split reached KNOWLEDGE 14 and not the map | map row 14 and the reconciliation table |
| F6 | Finding 6: the drift tally is not a partition — it sums to 62 over 59 rows | **accept** | All 59 drift cells parsed here: 25 contain "magnitude changed", 24 "compatible", 14 "not measured", 5 "sign or order changed", 1 "unresolved" — 69 tags, because 10 rows are compound (11, 14, 15, 21, 23, 30, 32, 37, 41, 42). The published 22 + 17 + 8 + 3 + 1 + 11 = 62. Under the map's own "strongest verdict" rule, made explicit as reversal → magnitude → unresolved → compatible → unmeasured, the assignment is **5 + 24 + 1 + 18 + 11 = 59**, the reviewer's numbers exactly; row 47 is missing from the named unmeasured list and 41's half is wrongly inside it | the map's tally, regenerated from explicit per-row assignments, "compound" an annotation |
| F7 | Finding 7: three comparator conclusions exceed their evidence | **accept** | (i) SaltZero's README carries no board-count distribution; in this project's own count-rule corpus the no-line third splits **16.5 % decided by count / 16.6 % equal** (`runs/plan6/I1_A6_corpus_stats.out:3`) — about half, not most, and a draw-rule agent has no reason to collect boards at all. (ii) SLAP's abstract, fetched: "SLAP reduced the number of training samples by a factor of 8 and achieved similar winning rate against the same evaluator, but it was not yet evident that it could speed up reinforcement learning". (iii) "augment, don't enforce" rests on one equal-cost implementation at one width (50) | `02_literature.md`: the SaltZero, SLAP and symmetry-row sentences |
| E2 | Fidelity entry **2**: τ, "the top four never change", "340 Elo" | accept with change | τ recomputed here over the 15 orbits at 16 384 sims from `runs/plan5_A1_atlas.json` and `runs/plan6/I1_A1_atlas.json`: `_e4` vs v2b **0.847619**, vs deep10 **0.885714**, vs deep8_300 **0.923810**; deep10 vs v2b 0.961905, deep8_300 vs v2b 0.923810 — the map's 0.85 / 0.89 / 0.92 and the 0.85–0.96 range stand. The same four orbits ([40] > [36] > [0] > [37]) are the top four in **all 12 columns**, but [0] and [37] swap at 1k and 4k on deep8_300 and `_e4` (orderings recomputed here), so the *order* claim holds at 16k only — KNOWLEDGE `:81–83` says so. The displayed τ span is v2b (0) → `_e4` (+363); 340 is dev1 → deep10 (43) | map row 2's statement and drift cell; KNOWLEDGE 2 |
| E3 | Fidelity entry **3**: "rises at every strength and has not stopped" | accept | `runs/plan5_A1_atlas.out:30` gives +0.354 / +0.455 / +0.447 at 16k; `runs/plan6/I1_A1_atlas.out:24` +0.495, and the gap to [36] is 0.4950 − 0.3950 = **+0.100**. deep8_300 → deep10 *falls* 0.008, so the sequence is not monotone, and "has not stopped" is an extrapolation from four points | map row 3's statement; KNOWLEDGE 3's "has not stopped" |
| E14 | Fidelity entry **14**: "six fits", and the drift cell reports A4 only | accept with change | Seven fits over the four strong nets — deep8_300 A4 + B3, deep10 A4 + B3, s1 B3 only, `_e4` A4 + B3 — plus v2b's A4 fit, eight in the coverage cell's own list. B3's verdict (all three below deep10's B3 intervals: 0.0209 / 0.0337 / 0.0316 at `I1_B3_value_decomp.out:51–56` against [0.0323, 0.1205] / [0.0474, 0.1108] / [0.0376, 0.0948] at `plan5_B3_value_deep10.out:36–41`) is R6's, accepted at the rebuttal and written into KNOWLEDGE, missing from the map | map row 14 |
| E15 | Fidelity entry **15**: *unresolved* is right; KNOWLEDGE's "unchanged to the second decimal since v2a" is not | accept | `plan5_B4_probe_value_deep10.out:14–16`: +1.052 / +0.941 / +0.827 (the map's 1.05 / 0.94 / 0.83); `runs/v2a/probe_surprise_rerun.out:15–17`: **+1.026 / +0.910 / +0.794** — the ordering survives, the magnitudes moved 0.026 / 0.031 / 0.033. Three different quantities are in play: A4's self-coefficients (centre > corner > edge), B3's (corner > centre > edge) and the self − opp contrast (deep10 centre +0.066, corner +0.090, edge +0.066, `plan5_B3_value_deep10.out:43`) | KNOWLEDGE 15; map row 15's level, coverage and file cells |
| E18 | Fidelity entry **18**: +0.017 is the centre board's removal, not "an open board" | accept | `plan5_B4_probe_value_deep10.out:19–21`: open **centre** removed +0.017 (n = 33 947, p10 −0.647, p90 +0.677), **corner** +0.012 (n = 133 431), **edge** +0.014 (n = 124 861). The row and KNOWLEDGE 18 both generalise the centre number, and neither carries the spread | KNOWLEDGE 18; map row 18 |
| E30 | Fidelity entry **30**: the `_e8` reading, and "dev and sealed agree within 0.3" | accept | J1a as in F3. The earlier agreement is 0.3 **to 1.3**: deep8_300 reads 83.5 on `endgame_v2_dev` (quoted inside KNOWLEDGE 46) against 84.8 [83.5, 86.0] sealed (KNOWLEDGE 30, which says "within 0.3 / 1.3") | map row 30 and the outline's §6 line |
| E31 | Fidelity entry **31**: the step is right, its label is not; the reviewer asks for *unresolved* on the diagnosis | accept with change; **reject** the drift change | `deep8_c1_300/timeline.json` iterations 200 / 210: `eg_draw_recognition` **0.588 → 0.657**; `_e4` **0.706 → 0.747**, 0.790 at 300; `_e8` 0.731 → 0.796, 0.810 at 300 — so the diagnosis cell's "≈ +6 on `_e4`" is the *paired score* against v2b (80.5 → 86.1, KNOWLEDGE 49), not draw recognition. The timeline's endgame metrics are read on `suites/endgame_v1.npz` (`timeline.json`'s `meta.endgame_set`; `tools/timeline.py:89, 112`), not on the 20 000-position corpus that KNOWLEDGE 51 names. *Unresolved* is refused: the level cell already says **inference** (M0 row 2), and the drift column records whether a reading recurs on a stronger net, not whether a mechanism is identified — §1a's five words are not the place to grade an inference | map rows 31 (metric) and 31 (diagnosis); KNOWLEDGE 51's set name |
| E32 | Fidelity entry **32**: rates right; "the same corpus" wrong; the bottom motifs swapped too | accept with change | `plan5_A7_puzzles_deep10_on_deep8late.out:3`: 126 (2.1 %) raw, 7 (0.12 %) search; `runs/plan6/I1_A7_puzzles.out:3`: 61 (1.0 %), 2 (0.03 %); `:5`: closes_board 4 / macro_win absent → macro_win 3 / closes_board absent. Stronger than the reviewer's "the outputs alone do not substantiate 'same corpus'": the samples are from **different** corpora — deep10's 6 000 from `runs/deep8_c1_300`'s last 20 iterations (`runs/plan5_A_3060.sh:15`), `_e4`'s 6 001 from `runs/deep10_c1_300`'s (`runs/plan6/I1_second_pass_3060.sh:74`, `HOLD=runs/deep10_c1_300`) | KNOWLEDGE 32; map row 32 |
| E33 | Fidelity entry **33**: the agent and solver columns are not paired, and `_e4`'s solver number is deep8_300's | accept | `runs/principles_*.json` read here: the agent's rate is keyed "search move (**unsolved** positions) …", the solver's "OPTIMAL move (**solved** positions, ≤ 14 empties) …" over 4 875 solved positions (deep10) and 4 791 (deep8_300 and `_e4`). deep8_300's and `_e4`'s solver values are the identical **0.7021498643289501** on the same 4 791 — not an independent replication at +363. `principles_deep10.json`'s sending data is `runs/probe_data_deep8late.npz`; `principles_deep8.json` and `I1_principles_deep8e4.json` read deep10-derived sets | KNOWLEDGE 33; map rows 33 (agent) and 33 (solver) |
| E35 | Fidelity entry **35**: numbers right, the attribution of the premium is not | accept | `plan5_B3_value_deep10.out:37` search +0.0102 ± 0.0418, raw −0.0484 ± 0.0412; `I1_B3_value_decomp.out:52` +0.0315 ± 0.0437 and −0.0598 ± 0.0459 — the map's rounded pairs agree. `plan5_B4_probe_value_deep10.out:17` gives centre − corner **+0.111**. Flipping one board's owner does not isolate the fourth line through it; it is consistent with the line count (4 / 3 / 2) | map row 35 (counterfactual); KNOWLEDGE 35 and 15's "in disguise" |
| J1 | 24, not 22, cells contain "compatible" | accept | The parse reproduces the reviewer's list exactly: 1, 8, 11, 13, 14, 17, 19, 21, 23, 26, 31 diagnosis, 31a, 33 agent, 33 solver, 34, 35 regression, 36, 37, 38, 40, 41, 42, 44, 45 | the map's tally (F6) |
| J2 | "inside the earlier interval" cannot govern all 24; label the rank / profile repetitions | accept with change | An earlier interval actually carries the verdict in **8** (deep10 +0.1960 ± 0.0281 → `_e4` +0.1953 ± 0.0278, `plan6/I1_A4_freemove.out:6`), **14** (A4), **19**, **35 regression**, **44** and **45**; the other eighteen rest on a repeated rank, share or profile with no interval quoted — 40's cell already says so. §1a fixes the five verdict words and this session does not add a sixth: every *compatible* cell is marked **[CI]** or **[no CI]** and the column definition says what the marks mean | map: the 24 cells, "Column definitions" |
| J3 | 17 does not establish "never moves" | accept | `I1_B3_value_decomp.out:2` (net_0010) search-value threats +0.123 / **−0.166** against `:42–43` (net_0300) +0.1310 ± 0.0265 / **−0.1363 ± 0.0201**: the threats_against coefficient drifts 0.030 across the run, outside the final interval, and does so steadily over the 30 checkpoints. "Learned by 10–20" holds; "never moves" does not | KNOWLEDGE 17; map row 17 |
| J4 | 7: "within 0.02" should be ≈ 0.02 | accept | `_e4` rates the edge subtree **+0.022** better where deep10 and deep8_300 rated the corner +0.016 better (KNOWLEDGE 7). 0.022 is not "within 0.02" | map row 7 (§1a carries the same phrase; left to the rebuttal round) |
| J5 | 48's gate: distinguish the widths | accept | Read here: narrow `gcnn8x16` dev policy KL **0.8057 / 0.8066** @3 120 (`G_arm_gcnn8x16.json`) against the ResNet's 0.8839 / 0.8853; **0.7875** @6 240 (`H4_lr_gcnn8x16_lr0.02.json`) against **0.8145** — still ahead; **0.8298** @12 480 (`H4_lr2_gcnn8x16_lr0.02_x32.json`) against **0.7630**. The parameter-matched `gcnn8x46` reads 0.7636 / **0.9195** / 1.0336 (`G_arm_gcnn8x46.json`) — *it* is the arm whose margin reverses by 6 240. The map's cell gives the wide arm's crossing to the narrow one; KNOWLEDGE 48 and 50 have it right | map row 48 |
| J6 | 42's G-CNN half is magnitude / architecture dependence, not a sign or order change | accept | The four ResNets' second-drop steps are +2.6 / +4.1 / −0.4 / −0.3, none resolved at ±2.8 (KNOWLEDGE 42, PLAN6 F2); the G-CNN's is a resolved +6.5 (KNOWLEDGE 50). An unresolved estimate straddling zero against a resolved positive establishes no flip, and both architectures order first drop ≫ second. The tally moves with it: **sign or order changed 5 → 4, magnitude changed 24 → 25** | map row 42 and the tally |
| J7 | 5's near-tie-to-separated reply change must stay enumerated whatever the ledger says | noted | Row 5's drift cell and §2 item 3 already carry it: a 0.003–0.006 tie on three nets → the self-send [20] at a gap of 0.073 on `_e4` | — |
| J8 | 20 / 24 / 25 / 27 are sensible descriptive reporting, not statistical drift detection; 32's swap is descriptive | noted | Their cells already say "magnitude changed" and quote no interval | — |
| L1 | Literature table, First move (`:190`): "best and worst agree" | accept | gPress tabulates five of the 15 orbits on an undocumented scale and calls every edge-board opening "bad"; it never ranks [13]. The agreement is on the best move and on the worst *class* | the row's reading cell |
| L2 | O's reply (`:191`) | accept | uttt.ai's is prose; there is no matched-budget engine experiment behind it | the reading cell |
| L3 | First-player edge (`:192`) | accept | The comparator is a bot arena's population, ours self-play with an exploration floor; the row keeps both differences explicitly | the ours / reading cells |
| L4 | Draws and the tiebreak (`:193`) | accept | `I1_A6_corpus_stats.out:3`: by line 67.0 %, by count 16.5 %, equal count 16.6 % — a third *reaches* the no-line terminal; the count *decides* 16.5 % | the ours cell |
| L5 | Game length (`:194`): "CLOSED" is incomplete | accept | Three sources of two or three variants under one tag — uttt.ai (CLOSED-DRAW, 40–50 plies), FLAIRS-35 ("at least 30", variant unstated), CG folk 50–60 (CLOSED-COUNT); §2a already lists them separately | the variant and source cells |
| L6 | Free move (`:195`): the integer 2 | accept | It is Lifshitz & Tsurel (HUJI 2016), a **course report** relayed on BGG/SE (`knowledge/07:311–315`), not a blog; the report's own link is dead, so its variant is unverified | the source and variant cells |
| L7 | Winnable board (`:196`) | accept | nelhage's is the narrow theorem (never send where one move wins *the game*); the 24.6 % is the agent's rate over unsolved positions and the 70.2 % the solver's over 4 791 solved ones (E33), so the pair is a juxtaposition, not a contrast | the claims / ours cells |
| L8 | Centre board (`:197`) | accept | The survey's weights are centre board 10, any board 5, corner board 3, centre square 3, free move 2 (`knowledge/07:311–315`): "10 vs 3" is centre-board against **corner**-board, additivity is not stated, and the variant is unverified | the row |
| L9 | Exploitability (`:198`) | accept with change | The thesis's variant is unverified and its page could not be retrieved (M0 row 29). A thesis is not a peer-reviewed publication, so the section's "nothing peer-reviewed touches CLOSED-COUNT" does not contradict it — but the row's "CLOSED [verify]" asserts more than is known | the variant cell → "unverified" |
| L10 | Solved status (`:199`) | accept | Bertholon is OPEN, confirmed from the paper itself (fetched): "it is not possible to win an already won field", and a free choice arises only when the field is *full*. "A frontier, not a value" over-reads sampled solved positions | the reading cell |
| L11 | Sample reuse (`:200`) | accept | Wang's sweep is 6×6 Othello, `ep` passes over a 20-iteration window; ours is passes over the new data inside a 7.6-iteration buffer — different games and different budget axes | the row, with F1 |
| L12 | Scaling (`:201`) | accept | Neumann & Gros train on **Connect Four and Pentago** and find strength a power law in parameters "when not bottlenecked by available compute"; being update-limited here does not exclude a parameter limit | the row |
| L13 | Symmetry (`:202`) | accept | SLAP is **Gomoku**, and its RL run kept the eightfold sample reduction at a similar winning rate (F7) | the row |
| L14 | LR schedule (`:203`) | accept | The warning is one maintainer's in leela-zero#1480, not a field prohibition; and the second drop is a resolved +6.5 on the G-CNN (KNOWLEDGE 50), so "only the first" is a ResNet statement | the row |
| L15 | Depth vs updates (`:204`) | accept | "Deeper is better" carries an *(arXiv)* source tag with no citation behind it; §2b itself calls it "AlphaZero-scale wisdom". It is folklore, and ours is conditioned on equal compute at this size | the source and claims cells |
| L16 | Concept probing (`:205`): "one trajectory" is false for Hex | accept | F2 | the row |
| S1 | Source check: SaltZero 113–87 and 96 / 200 | accept | README fetched: "After 200 games played with a fixed 400ms per move, SaltZero v0.0.3 beat NinjaDoggy's bot with a score of 113 to 87; detailed results: + 65 = 96 - 39" — a **score**, since 65 + 96/2 = 113 and 39 + 48 = 87; draws 96 / 200 = 48 % | the Agents paragraph and the table row |
| S2 | Source check: SLAP 83 %, one-eighth, AISB 2023 | accept | Abstract fetched (arXiv:2301.04746; AISB Convention 2023, pp. 69–75): "improved the convergence speed … by 83% … with only one eighth of the sample size compared with data augmentation" | the symmetry paragraph |
| S3 | Source check: Jones 500 Elo per decade | accept | §IV-A fetched: "The slope of the incline is 500 Elo per order of magnitude increase in compute", in "the linearly-increasing regime", on training compute, Hex 3×3–9×9; 500·log₁₀2 = **150.515** per doubling | the scaling sentence |
| S4 | Source check: Bertholon 43 moves / 29 rounds, OPEN | accept | Abstract and rules fetched: "at most 43 moves", "the second player can hold on at least 29 rounds", and the OPEN rule quoted in L10 | no change beyond L10 |
| S5 | Source check: Wang's reuse conversion | accept | R1's arithmetic re-derived by hand: Σ_{t ≤ I} min(t, 20) = 20I − 190, so R_I = ep(20 − 190/I); I = 25 → 12.4·ep = 62–186, I = 75 → 17.467·ep = 87–262, against a nominal 20·ep = 100–300 | `02_literature.md`: both forms, the `[being confirmed]` marker cleared |
| H1 | Hostile sentence 1 — row 2, outline `:169`: "a fact about the game, not the net" | accept | The reviewer's replacement is right and its numbers check (E2). Written as: at 16 384 simulations the search-relative first-move rankings of v2b, deep8_300, deep10 and `_e4` keep the same top four, with cross-net τ 0.85–0.96 and no sampling interval | map §3, §1's row-2 line |
| H2 | Hostile sentence 2 — row 19, `:186`: "an own local win is worth nothing alone" | accept | deep10 **−0.0259 ± 0.0168** (`plan5_B3_value_deep10.out:30`), `_e4` **−0.0367 ± 0.0175** (`I1_B3_value_decomp.out:45`) — both intervals exclude zero. It is a small resolved *negative* adjusted association, not nothing | map §3; KNOWLEDGE 19 (K7) |
| H3 | Hostile sentence 3 — row 18, `:189`: "a dead open board is worth nothing" | accept | +0.017 mean over **33 947** edits with p10 −0.647 and p90 +0.677 (E18); no interval on the mean is reported | map §3 and row 18 |
| H4 | Hostile sentence 4 — row 31a, `:211`: "solved outright … a tablebase adds nothing" | accept | `runs/plan6/I1_C5_tablebase_grade.out:1–5`: 689 of 60 000 positions (1.1 %), raw value 100.0 %, raw policy 100.0 %, 64-sim search 100.0 %; deep10's 701 of 60 000. True of these nets on these sampled positions; no population interval and no universal correctness follow | map §3; KNOWLEDGE 31a (K9) |
| H5 | Hostile sentence 5 — row 41a, `:230`: "named concepts carry none of the strength" | accept | `runs/paired_surrogate_vs_v2b_64.json` `summary.overall`: score 0.02180 → **2.18 % [1.36, 3.05]**, elo **−660.77 [−744.65, −600.76]**. It tests this 20-feature surrogate, not every representation over these concepts | map §3; KNOWLEDGE 41a (K8) |
| X1 | Row 6's outline clause: "training learns all of this first" | accept | Row 6 measures `first_top_share`, the raw head's probability on one move (`tools/timeline.py:130`; M2 row 7) — not the book, not the ordering | map §3, §1's row-6 line |
| X2 | The "three guards passed" assertion (`:247`) | accept | 38a exists for `runs/deep10_c1_300` alone and 41a is deep10's surrogate, so no claim re-read on `_e4` can have passed all three there; and decodability controls are not per-concept causal validation | map §3's Method bullet |
| X3 | Coverage spot-check: 37 has four nets, 39 three, 40 three, 36 two, 38a and 41a deep10 only; 13's "four strengths" includes v2b, and 8 has three strong nets | noted | Each matches the map's coverage cell as written; the only place the outline over-reaches is X2 | — |
| X4 | §4: the outline is an evidence catalogue in KNOWLEDGE order, not a paper | accept | Rules, utility units, corpus construction, budgets, strength calibration and PLAN7 §0's limits belong before the game claims; the seed band (a training-seed quantity) and the ±2.8 match interval are different things; the full map belongs in an appendix | map §3 gains a setup-and-limits head and an appendix line |
| X5 | The ±30 % conversion warning disappears from the outline's "a field that sits at ≈ 1" | accept | `02_literature.md:93–96` flags the AlphaGo Zero / AlphaZero / ELF / MiniZero figures as the survey's conversions ± 30 %; only Lc0 and KataGo publish a ratio | map §3's §9 bullet |
| X6 | `:175` — "five to ten orders higher" stated as fact | accept with change | 10³³–10³⁸ is this project's heuristic estimate (`knowledge/06` §2, marked heuristic after M0 row 30); the comparison with Othello's ≈ 10²⁸ follows *from that estimate*, so it is conditioned rather than deleted | `02_literature.md` |
| X7 | Neumann & Gros's venue can be given as ICLR 2023 | accept | Verified independently here, not on the reviewer's word: dblp `conf/iclr/NeumannG23`, the ICLR 2023 poster page, arXiv:2210.00849; games Connect Four and Pentago | `02_literature.md` |
| K1 | KNOWLEDGE 14 `:242`: "significant in three" | accept | Four of `_e4`'s **six coefficients across two fits** (A4's model and B3's, three self-ownership coefficients each; "six fits" was this table's own slip — M1 rebuttal) exclude zero: A4 +0.0489 ± 0.0388 and +0.0382 ± 0.0271 (`I1_A4_freemove.out:38, 42`, both starred by the tool), B3 +0.0337 ± 0.0336 and +0.0316 ± 0.0306 (`I1_B3_value_decomp.out:53, 55`) — the corner's lower endpoint is **+0.0001**. The fragility is the fact worth printing | KNOWLEDGE 14 |
| K2 | KNOWLEDGE 31 `:395–400`: the causal headline still sits under *Exact* | accept | The claim's level clause reads "*Exact; deep8_300 and deep10 timelines*" under the headline "'Draw blindness' was never a capacity limit". M0 row 2 split measurement from diagnosis in §1a and in the map, and never in the claim | KNOWLEDGE 31 |
| K3 | KNOWLEDGE 35 `:451`: the rejected headline remains | accept | "Conceding the centre board … costs what its lines cost, no more" is the phrase M0 row 8 replaced with "no resolved residual in this regression"; the map carries the correction, the claim does not | KNOWLEDGE 35 |
| K4 | KNOWLEDGE 48 `:742`: "capacity and not the LR" | accept | R4 named §0, §2b, KNOWLEDGE 50, README and RETROSPECTIVE; this sentence inside 48 was not found then and still asserts the mechanism | KNOWLEDGE 48 |
| K5 | KNOWLEDGE 50 `:854–856`: "the width is not what cost it the 220 Elo" | accept with change | `gcnn8x46` is a **supervised** arm on a frozen teacher set (2 459 392 parameters, 7.0× inference, dev KL 0.7636 / 0.9195 / 1.0336); no self-play run exists at that width, so the sentence draws a self-play conclusion from frozen-data curves. Restated as what the curves show, with the missing run named | KNOWLEDGE 50 |
| K6 | KNOWLEDGE 51 `:883–910`: "eight … is still not the plateau" | accept | +40 [+23, +57] resolves that *four* passes was not the plateau; nothing in the run bears on eight, and §10 already predicts the successor inside the seed band. The direct result is confirmed: recomputed from the 516 opening records of `runs/deep8_c1_300_e8/paired_vs_deep8c1_300e4_64.json`, score 0.5576550387596899 = **55.7655 %**, −400·log₁₀(1/s − 1) = **40.2418** Elo | KNOWLEDGE 51 |
| K7 | KNOWLEDGE 19's "worth nothing by itself" | accept | H2's two intervals, both excluding zero and both negative | KNOWLEDGE 19; map row 19 |
| K8 | KNOWLEDGE 41a carries no intervals | accept | `runs/paired_surrogate_vs_*.json` `summary.overall`: vs v2b 2.18 % [1.36, 3.05], −660.77 [−744.65, −600.76]; vs deep10 0.44 % [0.10, 0.87], −943.4 [−1199.8, −822.3]; vs the 10 000-playout rollout 14.29 % [12.16, 16.57], −311.2 [−343.5, −280.8] | KNOWLEDGE 41a |
| K9 | KNOWLEDGE 31a's "a tablebase … has nothing to add" | accept with change | H4's reading is of 701 and 689 *sampled* one-open-board positions; the sentence is true of these nets on those samples and is scoped to them | KNOWLEDGE 31a |

**What the review got right that was not asked.** It re-derived the atlas τ from the two JSON files
rather than quoting the map, and its six decimals match this session's recomputation digit for digit.
It found the asymmetry inside claim 33 that four earlier passes and two reviews had not — the agent's
quarter is measured on *unsolved* positions and the solver's 70 % on *solved* ones, so the two
columns of the most-quoted folk-rule result were never a paired comparison. It read the p10–p90 spread
behind an 0.017 mean instead of the mean alone. And its five surviving sentences are usable as
written: four of the five are copied into the outline unchanged.

**Open for M1's rebuttal round** (next Codex window, `exec resume` on its thread; its session id is the
first line of `docs/reviews/M1_account/events.jsonl`):

1. **E31.** The drift column is a re-read verdict, not a grade for an inference: 31's diagnosis is
   already marked *inference* by M0 row 2, and its step recurs (`_e4` draw recognition 70.6 → 74.7
   across the first drop, `_e8` 73.1 → 79.6). Does *unresolved* say anything the level cell does not,
   and if the answer is that "compatible" reads as support, is the **[CI] / [no CI]** mark (J2) enough?
2. **J2.** §1a's five verdicts are fixed, so the rank/profile cells are marked rather than renamed.
   Are the six **[CI]** rows the right six — 8, 14, 19, 35 regression, 44, 45?
3. **J6.** Row 42's G-CNN half was re-bucketed from *sign or order changed* to *magnitude changed*
   because the four ResNet second drops (+2.6 / +4.1 / −0.4 / −0.3) resolve nothing to reverse. Does
   the reviewer read a flip anywhere in that cell?
4. **E32.** Stronger than the finding as written: deep10's puzzles are sampled from deep8_300's late
   games and `_e4`'s from deep10's, so they are different corpora, not different samples of one. Does
   anything in the entry's reading depend on the corpora being the same?
5. **L9.** The exploitability row's variant becomes "unverified" rather than "CLOSED [verify]". Is
   there an archived copy of the Padua thesis, or of the HUJI course report behind the free-move
   integer (L6), that would settle either variant?
6. **X6.** The state-count comparison is kept, conditioned on this project's own heuristic estimate.
   Is a conditioned comparison worth keeping at all, or should the sentence go?

**M1's rebuttal round (`docs/reviews/M1_account/REBUTTAL.md`; thread `01a0989d-55ba-7930-866b-3aa405b8bd03`
resumed at `0691716`, 2026-09-13 03:08–03:16, reasoning effort medium: 475 s, 4 997 361 input tokens of
which 4 662 144 cached, 18 049 output, 12 commands, 15 MCP calls, 4 web searches; 922 words; the first
attempt at 23:49 hit the usage limit).** Six questions put; seven sections returned. Every evidence cell
below was re-derived here before its verdict — each `timeline.json` and paired JSON reopened and the
number recomputed, both primary documents fetched. The `_e8` count-rule pass's reading (37 dated clauses,
`runs/plan7/K1_parent_reading.md`) had landed an hour earlier, so the counts below are regenerated over
it, not over the table above.

| # | the reviewer's reply | verdict | evidence / reason | lands in |
|---|---|---|---|---|
| R1 | E31: *unresolved* withdrawn — **compatible [no CI]** is adequate and no sixth verdict is needed; but neither the mark nor the level cell repairs the categorical "It was optimisation, not capacity." Replace it with "Draw recognition improves after the first LR drop across these runs, consistent with an optimisation explanation; capacity limitations are not excluded", and finish the metric correction in row 31's strength cells | **accept** | Re-derived from the three files: `eg_draw_recognition` at iterations 200 → 210 is **0.588 → 0.657** (deep8_300, +6.9 points), **0.706 → 0.747** (`_e4`, +4.1), **0.731 → 0.796** (`_e8`, +6.5) — the reviewer's 6.9 / 4.1 / 6.5 exactly — and all three carry `meta.endgame_set = suites/endgame_v1.npz`. The rejected half of the M1 row stands (the drift column records recurrence, not mechanism); the accepted half is that a recurrence verdict cannot be printed under a causal headline. `_e4`'s cell said "first drop ≈ +6" (49's paired score) and `_e8`'s displayed endgame WDL 87.0 → 89.9 | KNOWLEDGE 31's headline and its diagnosis clause; map row 31 (diagnosis): the claim sentence, and the `_e4` / `_e8` cells now **70.6 → 74.7 %** and **73.1 → 79.6 %** |
| R2 | J2: move **row 44 to [no CI]** — its gains against deep10 rise +86 [67, 105] → +141 [121, 160] → +186 [165, 208], each later point estimate outside its predecessor's interval, so the row's *compatible* is the qualitative deployment conclusion; match CIs somewhere in a row do not satisfy the column's criterion | **accept** | `summary.overall` of the three paired JSONs read here: `_e2` **85.8725 [67.4956, 104.7424]**, `_e4` **140.5084 [120.7909, 160.3073]**, `_e8` **185.9453 [165.2907, 207.8775]**, each 1 032 games against `deep10_c1_300`. 140.51 > 104.74 and 185.95 > 160.31 — outside, as claimed. The row's bucket does not move (still *compatible*); only the mark does | map row 44; the tally's [CI] list |
| R3 | J2: **row 45 keeps [CI]** — `_e2`'s saved JSON gives 10.7767 Elo inside deep10's earlier [−7, +26]; describe that inclusion rather than reading two failures of the adoption rule as equivalence. The interval-based count becomes **five**, four in the compatible bucket and one inside row 14; broaden "[no CI]" to mean *no interval-based drift comparison* | **accept with change** | `runs/deep8_c1_300_e2/paired_phased_vs_256.json` `summary.overall`: elo **10.77665118361176**, ci [−5.7238, 26.3098], score 0.5155 [0.4918, 0.5378] — 51.6 %, the row's figures. deep10's is `runs/plan5_A8.out:27` "all 516 51.4 % [48.9, 53.7] +10 [−7, +26]"; 10.7767 is inside it. **The change:** five is the pre-`_e8` count. The count pass gave row **13** an interval-based verdict (`_e8` compatible [CI] against `_e4`'s [+0.127, +0.169] / [−0.150, −0.114]) and row 13 sits in the compatible bucket, so after 44 moves the bucket carries **8, 13, 19, 35 (regression), 45** — five, with three more interval-based *compatible* components inside compound cells (9, 14, 16). Eight [CI] cells in all, counted from the rows | map: the "Column definitions" drift entry, the tally's compatible bullet |
| R4 | J6: **no supported flip** — the G-CNN result changes magnitude and the second-drop conclusion, not the sign; first-drop improvement exceeds second-drop improvement in both architecture families. 4 / 25 / 1 / 18 / 11 stands. Keep the distinction between uncertainty on individual checkpoint scores and uncertainty on their difference: ± 2.8 is not a calculated CI for each drop | **accept** | The re-bucketing stands as adjudicated. The partition is now **4 / 26 / 1 / 20 / 8 = 59** — the count pass's regeneration an hour earlier, not a change from this round (12, 18 and 35's counterfactual left *not measured*). On ± 2.8 the reviewer is right and the map did not say so: `runs/plan6/F2_second_drop.out` quotes per-checkpoint scores against v2b @64 with their own intervals (net_0280 75.3 [72.8, 77.7], net_0300 75.2 [72.8, 77.4] — half-widths 2.45 and 2.3), so ± 2.8 bounds a single score, not a difference of two | map row 42's drift cell: the distinction written in |
| R5 | E32: nothing in the entry-32 reading depends on identical corpora; the different corpora *strengthen* the qualification — network, visited-position distribution and selected puzzle set change together. A frozen set would settle a within-set network comparison; a crossed evaluation would reveal corpus dependence; neither is required to report the descriptive difference | **accept** | Agreed and already on disk (row 32 and KNOWLEDGE 32 carry "different corpora, not paired" with both launcher lines). One fact the reviewer could not have: the `_e8` pass's puzzles are **the same 6 001 candidates from the same 3 076 games** as `_e4`'s — `runs/plan7/K1_parent_A7_puzzles.out:1–2` and `runs/plan6/I1_A7_puzzles.out:1–2` agree line for line, including exact W/D/L 0.440 / 0.154 / 0.405 — so `_e4` → `_e8` is the first *paired* puzzle reading in the chain, 61 of 6 001 against 52 of 6 001. The reading agent had already written that into row 32's `_e8` cell | — (row 32 already says it) |
| R6 | L6 / L9: the **HUJI report** recovered from the Internet Archive — CLOSED-DRAW, and §V confirms the free-move weight 2, the board-win weight 5, and that the weights were "chosen somewhat arbitrarily". Replace "unverified" for this comparator; keep its course-report status | **accept** | Fetched here, in full, at the reviewer's archive URL. §II: "The game ends when one of the players gets three symbols in a row in the large board, or in a tie if all squares have been exhausted" — a draw terminal, no count tiebreak — and "If all squares in the small board have been exhausted, or if the board has already been won, the player may choose to make his move anywhere on the board". §V *heur2*: "small board wins add 5 points, winning the center board adds 10, winning a corner board adds 3"; §V *heur4*: "if you are sent to a small board that is full or won you can play anywhere, so that add 2 points to the heuristic (and -2 for the other player)" — heur4 builds on heur3, which is heur2 memoised, so the 2-against-5 comparison is internal to one evaluator; and "The weights of different features of these heuristics were chosen somewhat arbitrarily". Course-report status confirmed from §I: "submitted as a final requirement for the Artificial Intelligence course". The report carries no year in its own text and the archive path reads `projects/2013/`, so the survey's "2016" is unconfirmed and the year is dropped rather than changed | `02_literature.md`: the free-move **and centre-board** rows (one document, both rows had "unverified (course report)") and the `:77–78` prose; `knowledge/07` §3.2 |
| R7 | L9: the **Padua thesis** recovered — §3.0.1 p. 28 establishes closed boards and §6.1 repeats it; the stated global victory condition is a line of won boards and no explicit no-line terminal adjudication was found. Mark **CLOSED; terminal tiebreak unverified**, not CLOSED-COUNT and not a fully verified CLOSED-DRAW | **accept** | Fetched here at the repository PDF URL. §3.0.1, p. 28: "if the required local board is already full or has been won, the next player can choose to play in any open cell on any available local board that has not been won or drawn" — won *and drawn* boards closed; §6.1: "they may play in any available position on any unfinished sub-board". Global condition: "The game is won by claiming three local boards in a row, column, or diagonal on the global board", with no adjudication for a finished game showing no such line. §6.1's reward is "1 for winning the global game, −1 for losing it, and 0 otherwise" and §7.1.1 reports "a stable draw rate", so draws occur and their rule is unstated. The reviewer's mark is exactly right | `02_literature.md`: the exploitability row and the `:49` prose; `knowledge/07` §2 |
| R8 | X6: conditioning makes the arithmetic honest but not the estimate informative; `knowledge/06` §2 multiplies independent per-board possibilities, omits inter-board reachability, and its lower endpoint is expressly not a lower bound. **Delete** "five to ten orders above Othello" from the manuscript; the heuristic may stay in the feasibility note without inferring solving cost | **accept** | `knowledge/06` §2 says it in its own words: "the lower end is a heuristic per-board reduction, not a bound — no defensible reachable-state *lower* bound is derived here", the bracket built as 4.4 × 10³⁸ × (5 478 / 19 683)⁹. A conditioned comparison still asks the reader to place UTTT on Othello's scale using a number the note refuses to defend | `02_literature.md`: the sentence deleted, with the reason in its place. `knowledge/06` untouched by instruction — and it still infers solving cost from the same bracket in §2 and §4 (open) |
| R9 | K5: the caveat is correct, but "the supervised curves do not put the 220 Elo down to the width" still invites causal exclusion. State only that the parameter-matched supervised arm also loses later; self-play width effects remain unmeasured | **accept** | The sentence did what the reviewer says: `gcnn8x46` is a frozen-teacher supervised arm (2 459 392 parameters, 7.0× inference, dev KL 0.7636 / 0.9195 / 1.0336, `runs/plan6/H4_lr*.json`), and no self-play run exists at that width — a negative about a cause cannot be drawn from it | KNOWLEDGE 50 |
| R10 | K9: the sampled qualification is correct; replace the surviving "solved outright" headlines in KNOWLEDGE 31a and the map with "Perfect measured accuracy on the sampled one-open-board positions" | **accept** | Both headlines found and replaced; the two were the only survivals in the repository (`KNOWLEDGE.md` 31a, `01_claims_map.md` row 31a; a grep over KNOWLEDGE, `docs/paper`, `knowledge/`, README and RETROSPECTIVE found no third) — the bodies already carried the scoping M1 added, over 701, 689 and 689 sampled positions of 60 000 | KNOWLEDGE 31a; map row 31a |
| R11 | "K1's 'six fits' should read 'six coefficients across two fits'" | **accept** | The table's own slip. `_e4` has two fits — A4's model and B3's — each yielding three self-ownership coefficients: A4 centre +0.0489 ± 0.0388, corner +0.0264 ± 0.0301, edge +0.0382 ± 0.0271 (`I1_A4_freemove.out:38–42`); B3 centre +0.0209 ± 0.0447, corner +0.0337 ± 0.0336, edge +0.0316 ± 0.0306 (`I1_B3_value_decomp.out:51–56`). Four of the six exclude zero and the corner's lower endpoint is +0.0001, as the cell said | §7e's K1 cell, edited. **Open:** KNOWLEDGE 14 carries the same slip twice ("`_e4`'s six fits", "positive in all six") from the count pass's own writing, outside this round's file list |
| R12 | §7 E2: accepted — membership holds across all twelve columns; fixed ordering requires the 16k qualification | **noted** | As adjudicated and already on disk (row 2). The reviewer's "twelve columns" is the pre-`_e8` count: rows 1 and 2 now read fifteen, and on `_e8` the top four hold their order at all three budgets, so the 16k qualification is `_e4`'s and not a standing one. Row 2's "in all 12 columns" is stale by three columns | — (outside this round's file list; noted for M3) |
| R13 | §7 E14: accepted — seven strong-net fits, eight including v2b | **noted** | Row 14's coverage cell already reads "seven fits over the four strong nets, eight with v2b's" | — |
| R14 | §7 L9: "I concede the distinction: a thesis does not contradict 'nothing peer-reviewed'. My original objection conflated that wording with the survey's broader academic claim" | **noted** | The concession matches the adjudication. `02_literature.md:47–48` keeps both sentences distinct — "nothing peer-reviewed touches CLOSED-COUNT" in the paper, against `knowledge/07`'s broader "nothing academic touches CLOSED/COUNT", which the thesis *does* bear on now that its variant is read as CLOSED | — |

**Open after the rebuttal:** the Padua terminal rule (only its code settles CLOSED-DRAW against
CLOSED-COUNT) and the HUJI report's year; `knowledge/06` still infers solving cost from the bracket the
paper has stopped quoting; KNOWLEDGE 14's "six fits" and row 2's "12 columns" are stale. M3 inherits all five.
