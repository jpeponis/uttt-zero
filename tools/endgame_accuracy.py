...
def main() -> None:
    ...
    for i in cand:
        ...
        if len(idx) >= a.n:
            break
    idx = np.array(idx)
    print(f"{len(idx)} distinct positions with {a.min_empty}-{a.max_empty} empties in open boards")

    t = time.perf_counter()
    exact = np.array([solve(cells_all[i], macro_all[i], int(nb_all[i]), int(pl_all[i]))[0] for i in idx])
    ...
    for s in [int(x) for x in a.sims.split(",")]:
        ...
        c2, m2, nb2, p2, d2, w2, _ = step_state(cells, macro, nb, player, done, winner, r.action)
        ...
        for i in range(n):
            if d2n[i]:
                ...
                child_val[i] = solve(c2n[i], m2n[i], int(nb2n[i]), int(p2n[i]))[0]
    ...
    if a.out:
        np.savez_compressed(a.out, idx=idx, exact=exact, empties=e)
